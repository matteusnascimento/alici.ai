...
function initializeApp() { initializeTheme();
    bindUI();
    initializeParticles();
    initializeChart();
    initializeLucide();
...
function bootLoader() { const loader = document.getElementById('appLoader');
...
function showSection(sectionId) { state.currentSection = sectionId;
    ...
    document.querySelectorAll('.nav-item').forEach(i => { i.classList.remove('active'); if (i.dataset.section === sectionId) { i.classList.add('active') } }); const active = document.getElementById(sectionId); if (active) { active.classList.add('active') } const title = document.getElementById('sectionTitle');
    title.textContent = { dashboard: 'Dashboard', chat: 'Chat IA', models: 'Modelos', analytics: 'Analytics', settings: 'Configurações' }[sectionId] || 'ALICI'; if (sectionId === 'chat') { autoScroll() } }

function appendMessage(content, role = 'assistant') { const messages = document.getElementById('chatMessages'); const el = document.createElement('article');
    el.className = `message ${role==='user'?'user-message':'assistant-message'}`; const paragraph = document.createElement('p');
    paragraph.textContent = content;
    el.appendChild(paragraph);
    messages.appendChild(el);
    autoScroll() }

function createTypingLoader() { const messages = document.getElementById('chatMessages'); const loader = document.createElement('article');
...
async function sendMessage() { const input = document.getElementById('chatInput'); const userMessage = input.value.trim(); if (!userMessage) { return }
    ...
    createTypingLoader(); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 15000); try { const response = await fetch('/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: userMessage }), signal: controller.signal }); let responseText = 'Recebi sua mensagem. Integre o endpoint para resposta em tempo real.'; if (response.ok) { const data = await response.json();
        ...
        showNotification('Erro ao conectar no endpoint /chat.', 'error') } finally { clearTimeout(timeout) } }
...
function showNotification(message, type = 'success') { const container = document.getElementById('notifications'); const toast = document.createElement('div');
    toast.className = 'toast'; if (type === 'error') { toast.style.borderColor = 'rgba(239,68,68,.45)' } if (type === 'warning') { toast.style.borderColor = 'rgba(245,158,11,.45)' }
...
function toggleTheme() { state.theme = state.theme === 'dark' ? 'light' : 'dark';
    ...
    localStorage.setItem('alici_theme', state.theme); const icon = document.querySelector('#themeToggle i');
    icon.setAttribute('data-lucide', state.theme === 'dark' ? 'moon' : 'sun');
    initializeLucide(); if (state.chart) { state.chart.destroy();
        initializeChart() } }

function initializeTheme() { document.documentElement.setAttribute('data-theme', state.theme) }

function initializeChart() { const canvas = document.getElementById('requestsChart'); if (!canvas || typeof Chart === 'undefined') { return } const light = document.documentElement.getAttribute('data-theme') === 'light'; const textColor = light ? '#334155' : '#94a3b8'; const gridColor = light ? 'rgba(148,163,184,.25)' : 'rgba(148,163,184,.16)';
...
function initializeParticles() { const canvas = document.getElementById('particlesCanvas'); const ctx = canvas.getContext('2d'); const particles = []; const quantity = 80; const resize = () => { canvas.width = window.innerWidth;
        canvas.height = window.innerHeight }; const make = () => ({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, radius: Math.random() * 1.8 + .4, speedX: (Math.random() - .5) * .28, speedY: (Math.random() - .5) * .28, opacity: Math.random() * .6 + .2 }); const draw = () => { ctx.clearRect(0, 0, canvas.width, canvas.height);
        particles.forEach(p => { p.x += p.speedX;
            p.y += p.speedY; if (p.x < 0 || p.x > canvas.width) { p.speedX *= -1 } if (p.y < 0 || p.y > canvas.height) { p.speedY *= -1 }
            ctx.beginPath();
    ...
    resize();
    window.addEventListener('resize', resize); for (let i = 0; i < quantity; i += 1) { particles.push(make()) }
    draw() }