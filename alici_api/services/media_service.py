"""Local media generation implementations used by background workers."""
...
def _safe_slug(text: str, max_len: int = 60) -> str:
    ...
    return (cleaned or "prompt")[:max_len]
...
def generate_video(prompt: str) -> dict:
    ...
    payload = {
        ...
        "message": "Geracao de video processada pelo provider local.",
    }
...
def analyze_image_bytes(content: bytes, filename: str, content_type: str) -> dict:
    ...
    return {
        "descricao": (
            "Analise basica concluida pelo provider local. "
