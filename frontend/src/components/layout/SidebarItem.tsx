...
export function SidebarItem({ label, to, icon: Icon, expanded, onNavigate }: SidebarItemProps) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          expanded ? 'h-14 min-h-14 justify-start gap-3 rounded-2xl border px-3' : 'mx-auto h-[3.25rem] w-[3.25rem] items-center justify-center rounded-2xl border p-0',
          isActive
    ...
    >
      <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-white/15 bg-ink/70 text-cyan shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
        <Icon size={19} strokeWidth={2.25} className="h-[19px] w-[19px] shrink-0" />
      </span>
