...
async function sendMessage() {
    ...
    try {
        const data = await api.post(`/chat/message`, { message: text, conversation_id: currentConvId });
        removeTyping(typingEl);

        const reply = data.content || data.message || data.response || data.resposta || "Sem resposta.";
...
async function openProfileModal() {
    ...
    try {
        ...
        if (photoEl && data.avatar_url) photoEl.src = data.avatar_url;
    } catch {}
}
...
async function uploadPhoto() {
    ...
    try {
        const data = await api.postForm("/user/avatar", fd);
        const photoEl = document.getElementById("profilePhoto");
        if (photoEl && data.avatar_url) photoEl.src = data.avatar_url;
